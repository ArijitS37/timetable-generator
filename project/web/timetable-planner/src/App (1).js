...
const exportToXLSX = () => {
  ...
  subjects.forEach(subject => {
    ...
    const coTeachers = assignment.coTeachers
    ...
    const notes = assignment.assistants?.length
      ? `Lab Assistants: ${assignment.assistants
          .map(id => TEACHERS.find(t => t.id === id)?.initials)
          .join(', ')}`
      : '';
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Timetable_Input');

  XLSX.writeFile(workbook, 'timetable_input.xlsx');
};
