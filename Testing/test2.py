...
slots = [f"{h}:30-{h+1}:30" for h in range(8, 18)]  # 8:30–6:30
...
# (4) Teacher load (max hours per week)
# teacher_loads = {teacher: random.randint(6, 12) for teacher in teachers}
# for teacher in teachers:
#     model.Add(
#         sum(
#             timetable[(subj["Subject"], t)]
...
#     print("\n✅ Timetable generated (sample output):")
#     for subj in subjects[:5]:  # just print 5 subjects as sample
#         subj_name = subj["Subject"]
#         print(f"\n{subj_name} ({subj['Teacher']}, {subj['Room_type']})")
#         for t in range(len(time_slots)):
